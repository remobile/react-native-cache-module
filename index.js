...
const xconsole = console;
const syncModuleSource = {};
...
class StorageMgr {
    constructor () {
        ...
        db.transaction((tx) => {
            tx.executeSql('CREATE TABLE IF NOT EXISTS ' + TABLE_CACHE_MODULE + ' (name constchar(40) primary key, size integer, time integer)');
        ...
        return new Promise((resolve, reject) => {
            db.transaction((tx) => {
                tx.executeSql('SELECT name, size FROM ' + TABLE_CACHE_MODULE + ' WHERE time=(SELECT MIN(time) FROM ' + TABLE_CACHE_MODULE + ')', [], (tx, rs) => {
                    if (rs.rows.length) {
                        const { name, size } = rs.rows.item(0);
